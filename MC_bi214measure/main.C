...
int main(int argc, char** argv)
{
	...
	TFile* f1=new TFile("contamination.root","RECREATE");
	...
	while(TR->fReader.Next() )
	{
		...
		Double_t time2first=(TR->T2PrevSubEvt)[1];
			...
			if(TR->D2First[1] <=300 ){
                        	if(/*true*/ /*(TR->Z[0]<=0 || TR->Z[1]<=0) &&*/ x>=40 && x<=80 && y>=50 && y<=250){
