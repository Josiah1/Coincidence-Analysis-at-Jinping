...
int main(int argc, char** argv)
{
	...
	//TFile* f1=new TFile("runrate_323.root","RECREATE");
	TFile* f1=new TFile("runrate_332_10s.root","RECREATE");
	TString inputfiledir="/work/wangzhe10_work/JinjingLi/JinpingPackage/20180527/JSAP-install/Analysis/PreAnalysis/src/output/";
	...
	while(TR->fReader.Next())
	{
		...
		if(currTime<=(begin_time+(segi+1)*time_interval) && nEntries<entries){
			...
			if(bValid) {
				pedecut_eventcounter+=1;
				pedeflashcut_eventcounter+=1;	
			}
			 //--------------
                         segi=segi+1+pop;
		}
	}
	TGraphErrors* gr1 = new TGraphErrors(60/*time_b.size()*/, &(time_b[0]), &(counter[0]), &(time_b_error[0]), &(counter_error[0]));
	...
	TGraphErrors* gr3 = new TGraphErrors(time_b.size(), &(time_b[0]), &(pedeflashcut_counter[0]), &(time_b_error[0]), &(pedeflashcut_counter_error[0]));
        ...
        gr1->GetXaxis()->SetNdivisions(2);
        gr1->GetXaxis()->SetTimeFormat("%Y/%m/%d/%H");
	...
	cgr1->Close();
        ...
        gr2->GetXaxis()->SetNdivisions(2);
        gr2->GetXaxis()->SetTimeFormat("%Y/%m/%d/%H");
	...
	cgr2->Close();
        ...
        gr3->GetXaxis()->SetNdivisions(2);
        gr3->GetXaxis()->SetTimeFormat("%Y/%m/%d/%H");
