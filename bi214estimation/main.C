...
double tau_sig=2.37271e-4;
double databegin=0.,dataend=2.5e-3;
...
void fcn(int& npar, double* deriv, double& f, double par[], int flag){
  ...
  double lnL = 0.0;
  for (int i=0; i<n; i++){
    double x = xVec[i]-1.5e-3;//for specical use; For normal use please change to:  double x = xVec[i];
...
int main(int argc, char** argv)
{
	...
	//TFile* f1=new TFile("upperlimitofbkg.root","READ");
	...
	*/
	...
	while(TR->fReader.Next() )
	{
		...
		Double_t time2first=(TimeStamp((TR->TrigSec)[1],(TR->TrigNano)[1])-TimeStamp((TR->TrigSec)[0],(TR->TrigNano)[0])).GetSeconds();
			...
			if(TR->D2First[1] <=300 ){
                        	if(x>=40 && x<=80 && y>=50 && y<=250 && time2first>=1.5e-3){
	...
	TF1* fbest=new TF1("fbest",lnLi,0,2.5e-3,2);
	Int_t bin=100;
	fbest->SetParameters(par1*(2.5e-5),par2*(2.5e-5));
	TH1D* hdata=new TH1D("hdata","",bin,0,2.5e-3);
	for(size_t i=0;i<xVecPtr->size();i++){
		hdata->Fill((*xVecPtr)[i]-1.5e-3);
	}
