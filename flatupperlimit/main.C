...
using namespace std;
double tau_sig=2.34859e-4;
double uniformConstant=250;
...
Double_t lnLi(Double_t* x,Double_t* par){
        ...
        Double_t f=par[0]*TMath::Exp(-xx/tau_sig)/tau_sig+par[1]*uniformConstant;
        return f;
}
...
int main(int argc, char** argv)
{
	...
	TFile* f1=new TFile("../bi214evolution/evolutioncheck.root","READ");
	TFile* file2=new TFile("result.root","RECREATE");

	TTree* t1=(TTree*)f1->Get("fittime");
	t1->SetBranchAddress("maptime3",&xVecPtr);	
	t1->GetEntry(0);
	...
	TF1* fbest=new TF1("fbest",lnLi,0,4.e-3,2);
	Int_t bin=100;
	fbest->SetParameters(par1*(4.e-5),par2*(4.e-5));
	TH1D* hdata=new TH1D("hdata","",bin,0,4.e-3);
	...
	TF1* ff1=new TF1("ff1","2.71",0,500);
	leg->AddEntry(f1,"q=2.71 corresponding to 90%C.L.","l");
